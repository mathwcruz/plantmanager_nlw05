...
export async function savePlant(plant: PlantProps) : Promise<void> {
  try {
    ...
    const differenceInSeconds = Math.abs(Math.ceil(now.getTime() - nextTime.getTime()) / 1000);

    const notificationId = await Notifications.scheduleNotificationAsync({
      content: {
        title: 'Heeeeey, 🌱',
        body: `It's time to take care of your ${plant?.name || 'plant'}`,
        sound: true,
      ...
      trigger: {
        seconds: differenceInSeconds < 60 ? 60 : differenceInSeconds,
        repeats: true,
      }
    })
